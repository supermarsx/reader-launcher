...
If ($cfg_debugnoexec = 0) Then
	...
	; If path is a folder then attempt to open it — otherwise check file existence
...
Func _EnsureLogDirExists($path)
	...
	Local $last = StringInStr($path, '\\', 0, -1)
	If $last = 0 Then Return
	Local $dir = StringLeft($path, $last - 1)
	If Not FileExists($dir) Then DirCreate($dir)
EndFunc

Func Log($level, $message)
	If $cfg_logenabled <> 1 Then Return
	Local $lvl = MapLevel($level)
	If $lvl > $cfg_loglevel Then Return
		...
		Switch $src
			Case "registry"
				Local $keys[2] = [ _
					...
					"HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\App Paths\AcroRd32.exe" _
				]
